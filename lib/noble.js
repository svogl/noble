...
Noble.prototype.purgeConnection = function(peripheralUuid, handle) {
  var peripheral = this._peripherals[peripheralUuid];

  if (peripheral) {
    peripheral.state = 'disconnected';

    this._bindings.purgeConnection(handle);
  } else {
    this.emit('warning', 'unknown peripheral ' + peripheralUuid + ' -- cannot purge.');
  }
};
